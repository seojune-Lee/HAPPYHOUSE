...
import com.ssafy.environment.EnvironmentView;
...
import com.ssafy.store.StoreView;
...
public class HouseInfoView{
	...
	private void showHouseInfo(int code) {
		...
		String imgName = null;
		if(houseDao.searchImg(curHouse.getDong()+curHouse.getAptName()) != null) {
			imgName = houseDao.searchImg(curHouse.getDong()+curHouse.getAptName());			
		}

		ImageIcon icon = null;
		if( imgName != null && imgName.trim().length() != 0) {
			icon = new ImageIcon("img/" + imgName);
	...
	public void setMain(){
		...
		houseTable.addMouseListener( new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				showHouseInfo(code);
				new EnvironmentView(curHouse.getDong());
				new StoreView(curHouse.getDong());
			}
		});
		
		houseTable.getTableHeader().addMouseListener(new MouseAdapter() {
			@Override
			public void mouseReleased(MouseEvent e) {
				int col = houseTable.columnAtPoint(e.getPoint());
				if(col == 1) {
					amountType = 1;
					if(dongType == 1) {
						dongType = 0;
...
//		searchBt.addActionListener( buttonHandler );
	...
	public void showSortedHousesByDong() {
		List<HouseDeal> deals = houseService.sortDong(dongType);
		if(deals!=null){
			int i=0;
			String[][]data = new String[deals.size()][5];
			for (HouseDeal deal: deals) {
	...
	public void showSortedHousesByAmount() {
		List<HouseDeal> deals = houseService.sortAmount(amountType);
		if(deals!=null){
			int i=0;
			String[][]data = new String[deals.size()][5];
			for (HouseDeal deal: deals) {
